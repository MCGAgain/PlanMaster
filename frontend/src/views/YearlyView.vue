...
function animateCards() { if (cardTween) cardTween.kill(); nextTick(() => { if (!planListRef.value) return; const cards = planListRef.value.querySelectorAll('.plan-card'); if (!cards.length) return; gsap.set(cards, { clearProps: 'clipPath' }); cardTween = gsap.from(cards, { clipPath: 'inset(0 0 100% 0 round 16px)', duration: 0.5, stagger: 0.06, ease: 'power2.out', onComplete: () => gsap.set(cards, { clearProps: 'clipPath' }) }) }) }
