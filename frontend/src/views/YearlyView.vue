<template>
  <div class="yearly-view">
    <Header title="年计划" searchable @search="handleSearch">
      <template #actions>
    ...
    <GlassCard v-if="filteredPlans.length === 0" class="empty-state"><p>暂无年计划</p></GlassCard>
    <GlassModal v-model="showAddModal" title="新增年计划"><PlanForm @submit="handleAdd" @cancel="showAddModal = false" /></GlassModal>
...
const plans = computed(() => plansStore.yearlyPlans)
...
onMounted(async () => { plansStore.setCurrentType('yearly'); await plansStore.fetchPlans(); await loadCategoryProgress(); await loadSignatures() })
...
const loadCategoryProgress = async () => { try { categoryProgress.value = (await api.getPlanProgress('yearly')).percentage || 0 } catch (e) {} }
...
const handleAiSort = async () => { try { await api.sortPlans('yearly'); await plansStore.fetchPlans(); await loadCategoryProgress() } catch (e) { alert('AI排序失败: ' + e.message) } }
...
const handleAdd = async (p) => { try { await plansStore.createPlan({ ...p, plan_type: 'yearly' }); showAddModal.value = false; await loadCategoryProgress(); showNextSignature() } catch (e) { alert(e.message) } }
