        ...
        <div v-for="(p, idx) in filteredPlans" :key="p.id" class="plan-card" :class="{ completed: p.completed }" :style="{ animationDelay: (idx * 0.04) + 's' }">
          ...
          <div class="plan-card-body">
            ...
            <div class="plan-card-meta">
              <span class="plan-type-tag" :style="{ background: '#f59e0b' }">年计划</span>
...
function animateCards() { if (cardTween) cardTween.kill(); nextTick(() => { if (!planListRef.value) return; const cards = planListRef.value.querySelectorAll('.plan-card'); if (!cards.length) return; gsap.set(cards, { clearProps: 'clipPath,willChange' }); cardTween = gsap.from(cards, { y: 16, clipPath: 'inset(0 0 100% 0 round 16px)', duration: 0.4, stagger: 0.04, ease: 'power3.out', onComplete: () => gsap.set(cards, { clearProps: 'willChange' }) }) }) }
const planDateLabel = computed(() => new Date().getFullYear() + '年')
...
const loadPlans = async () => { try { const [active, progress] = await Promise.all([api.getPlans(planType), api.getPlanProgress(planType).catch(() => ({ percentage: 0 }))]); plans.value = active; categoryProgress.value = progress.percentage || 0; animateCards() } catch (e) { window.toast('加载失败: ' + e.message, true) } }
