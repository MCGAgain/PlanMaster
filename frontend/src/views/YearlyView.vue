...
function animateCards() { if (cardTween) cardTween.kill(); nextTick(() => { if (!planListRef.value) return; const cards = planListRef.value.querySelectorAll('.plan-card'); if (!cards.length) return; gsap.set(cards, { clearProps: 'clipPath,willChange' }); cardTween = gsap.from(cards, { y: 16, clipPath: 'inset(100% 0 0 0)', duration: 0.4, stagger: 0.04, ease: 'power3.out', onComplete: () => gsap.set(cards, { clearProps: 'willChange' }) }) }) }
