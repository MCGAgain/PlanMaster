<template>
  <div class="weekly-view">
    ...
    <GlassCard v-if="filteredPlans.length === 0" class="empty-state"><p>暂无周计划</p></GlassCard>
    <GlassModal v-model="showAddModal" title="新增周计划"><PlanForm @submit="handleAdd" @cancel="showAddModal = false" /></GlassModal>
...
const filteredPlans = computed(() => {
  ...
  return plans.value.filter(p => matchPinyin(p.title, searchQuery.value) || matchPinyin(p.description || '', searchQuery.value))
})
onMounted(async () => { plansStore.setCurrentType('weekly'); await plansStore.fetchPlans(); await loadCategoryProgress(); await loadSignatures() })
...
const loadCategoryProgress = async () => { try { categoryProgress.value = (await api.getPlanProgress('weekly')).percentage || 0 } catch (e) {} }
...
const handleAiSort = async () => { try { await api.sortPlans('weekly'); await plansStore.fetchPlans(); await loadCategoryProgress() } catch (e) { alert('AI排序失败: ' + e.message) } }
...
const handleAdd = async (p) => { try { await plansStore.createPlan({ ...p, plan_type: 'weekly' }); showAddModal.value = false; await loadCategoryProgress(); showNextSignature() } catch (e) { alert(e.message) } }
